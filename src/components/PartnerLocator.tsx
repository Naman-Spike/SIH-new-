...
export default function PartnerLocator({
  ...
  const handleGeocodeSearch = async (queryText?: string) => {
    const query = (queryText || selectedCity).trim();
    if (!query) return;
    ...
    try {
      ...
      // Fallback already handled inside geocodeCity
    } finally {
  ...
  return (
    <div className="w-full space-y-8">
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full pl-4 pr-10 py-2.5 font-bold text-sm border-2 border-black rounded-full focus:outline-none focus:ring-2 focus:ring-black"
                />
              </div>
              <button
                type="button"
                ...
                className="px-4 py-2.5 bg-black hover:bg-zinc-800 text-white font-bold text-xs rounded-full flex items-center gap-1.5 transition-colors disabled:opacity-50"
                title={t('locatorSearchCityBtn')}
              >
                {isSearchingCity ? (
                ...
                <span className="hidden sm:inline">{t('locatorSearchCityBtn')}</span>
              </button>
            </div>
      ...
      {/* Interactive Geo-Spatial Map (Leaflet / OpenStreetMap / Overpass API) */}
      <div className="bg-white border-2 border-black rounded-3xl p-6 md:p-8 shadow-sm space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-3 text-xs font-semibold">
            ...
            <span className="flex items-center gap-1.5">
              ...
              {isHindi ? 'बहिष्कृत (उच्च NPA)' : 'Excluded (>5% NPA)'}
            </span>
          </div>
        </div>
        ...
        <PartnerMap
          ...
          onSelectPartner={(partner) => setSelectedPartnerForVoucher(partner)}
        />
      </div>
